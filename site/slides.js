...
window.SLIDES = (() => {
  ...
  return [
    ...
    },
    {
      ...
      content: html`<div class="callout"><h3>Dados do problema</h3><p>Fixamos uma região \(\Omega\subset\mathbb{R}^2\), um raio de comunicação \(R_{\text{com}}>0\), uma ponto \(\sigma\in\Omega\) sink indexado por \(s\), posições candidatas \(Q=\{q_j\in\Omega\mid j\in\mathcal J\}\) e dispositivos móveis com trajetórias conhecidas<br> \(\gamma_m:[0,H]\to\Omega\), \(m\in\mathcal M\).</p></div><p>O horizonte é discretizado em períodos \(t\in\mathcal T\). Em cada período:</p><div class="equation">\[G_t=(V,E_t),\qquad V=\{s\}\cup\mathcal J\cup\mathcal M\]\[E_t=\big\{(i,j)\in V\times V\mid0&lt;d_{ij}(t)\le R_{\mathrm{com}}\big\}.\]</div><div class="callout blue"><p>Selecionar \(P\subseteq Q\) para que, em cada período, a demanda dos móveis chegue ao sink por caminhos com capacidade suficiente.</p></div>`
    },
